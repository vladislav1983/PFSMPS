...
register int DSP_ACCU_B asm("B");   // DSP ENGINE ACUMULATOR "B"
/*=====================================================================================================================
...
U16 ATAN(S16 iY, S16 iX)
{
    ...
    if (iX >= 0)
    {
        if (iY >= 0)      
        {
            ...
            {
                iArgu = ((((U32)iY<<16)>>1) / (iX+1));  	// iArgu = (iY * 32768) / iX. Add 1 to iX, to avoid iArgu = 0.
        ...
        else
        {
            ...
            { 
                iArgu = ((((U32)iY<<16)>>1) / (iX+1)); /* Add 1 to avoid overflow*/
                uArguComma = iArgu<<(DEC_PLACES_BEFORE + 1);
                iArgu    >>= DEC_PLACES_AFTER;
                iTemp      = ATAN_TAB[iArgu];
...
Q15 fmul_q15(Q15 qA, Q15 qB)
{
    ...
    // Disable interrupts for cycles required for fractional computation
    __builtin_disi(6);  // Disable level 6 interrupts for (lit14 + 1) instruction cycles
    // Save CORCON
    asm volatile ("push CORCON");
    // Setup DSP engine
    CORCONbits.IF = 0;  // Put DSP Engine in fractional mode
    CORCONbits.US = 0;	// Put DSP In Signed Mode
    // do the multiplication.. DSP_ACCU_A stored in 40 bit accumulator
    DSP_ACCU_A = __builtin_mpy((Q15)qA, (Q15)qB, NULL, NULL, 0, NULL, NULL, 0);
